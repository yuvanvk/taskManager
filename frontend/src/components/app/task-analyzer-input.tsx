...
export const TaskAnalayzerInput = () => {
    ...
    const handleAnalyze = async () => {
       const response: AxiosResponse = await axios.post("http://localhost:8000/api/tasks/analyze/", {
            tasks,
       })
       ...
       const getTopThreeTasksResponse = await axios.get("http://localhost:8000/api/tasks/suggest/")
       if(getTopThreeTasksResponse.status === 200) {
        setTasks(c => [...getTopThreeTasksResponse.data.top_three_tasks])
        setAnalyzed(true)
       }
       if(getTopThreeTasksResponse.status !== 200) {
    ...
    return <div className="flex flex-col w-full my-5">
        ...
        <div className="my-4 space-y-2">
            ...
            <p className="text-xs text-neutral-500">Optimal algorithm balancing all factors for maximum efficiency</p>
        </div>
